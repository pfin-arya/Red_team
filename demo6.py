...
# 1) USER CONFIG
# =========================================================
# You can provide exact file with extension OR extension-less base path.
# Example:
# API_DOC_PATH = r"C:\...\Red_team_1\pai_chat.md"
# API_DOC_PATH = r"C:\...\Red_team_1\API Documentation - RegIntel-case1"
API_DOC_PATH = r"C:\Users\CINT037\OneDrive - Poonawalla Fincorp Limited\Desktop\Red_team_1\API Documentation - RegIntel-case1"
...
API_BASE_URL_OVERRIDE = ""  # e.g. https://<funcapp>.azurewebsites.net
FUNCTION_KEY_OVERRIDE = ""  # if API requires Azure Functions key
...
def mask_secret_headers(headers: Dict[str, str]) -> Dict[str, str]:
    ...
    for k, v in headers.items():
        lk = k.lower()
        if "authorization" in lk or "key" in lk or "token" in lk:
            masked[k] = "***"
        else:
            masked[k] = v
    return masked
...
def resolve_doc_path(path_str: str) -> Path:
    ...
    # If path has no suffix, try common ones
    if p.suffix == "":
...
def build_runtime_api_config(api_meta: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Apply base URL override if endpoint is relative
    cfg["url"] = normalize_url(cfg["url"], API_BASE_URL_OVERRIDE)

    # Function key override support
...
def call_target_api(user_prompt: str, runtime_cfg: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if method == "GET":
        response = requests.get(
            url,
            headers=headers,
            params=payload if isinstance(payload, dict) else {"message": user_prompt},
        ...
        response = requests.request(
            ...
            url,
            headers=headers,
            json=payload if isinstance(payload, (dict, list)) else {"message": user_prompt},
            timeout=REQUEST_TIMEOUT,
            verify=VERIFY_SSL
        )
...
def run_test_cycle(api_meta: Dict[str, Any], runtime_cfg: Dict[str, Any]) -> Tuple[List[Dict[str, Any]], Dict[str, Any]]:
    ...
    for i in range(NUM_SEEDS):
        try:
            ...
            try:
                fups = generate_followups(seed, seed_call["response_text"], api_meta)
            except Exception:
                fups = []
    ...
    if not all_prompts:
        raise RuntimeError(
            "No prompts generated. Check LLM extraction/generation and API config.\n" +
            "\n".join(seed_errors[:5])
        )
