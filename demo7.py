...
def llm_chat(model: str, prompt: str, system: Optional[str] = None, temperature: Optional[float] = None) -> str:
    try:
        ...
        # Some models (e.g., gpt-5-mini deployments) reject explicit temperature values.
...
def sanitize_url(raw_url: str) -> str:
    """
    Remove accidental quote artifacts from extracted URLs while preserving query params.
    """
...
def inject_prompt_into_body(template_body: Any, prompt: str, candidates: List[str], required_fields: List[str]) -> Any:
    """
    Strict request shaping:
    - If template exists, update only template keys.
    - Do not auto-add extra fields that may break schema validation.
    """
